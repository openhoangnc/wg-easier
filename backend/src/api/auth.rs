...
pub async fn login(
    ...
    if let Some(sid) = get_session_id_from_headers(&headers) {
        if let Some(username) = state.sessions.read().unwrap().get(&sid).cloned() {
            return Ok((
                StatusCode::OK,
                HeaderMap::new(),
                Json(SessionResponse { authenticated: true, username: Some(username) }),
            ).into_response());
        }
    }
    ...
    if !state.config.insecure {
        ...
        // TOTP check
        if let Some(user) = crate::db::users::find_by_username(&state.db, &body.username).await
    ...
    state.sessions.write().unwrap().insert(session_id.clone(), body.username.clone());
    ...
    Ok((
        StatusCode::OK,
        resp_headers,
        Json(SessionResponse { authenticated: true, username: Some(body.username) }),
    ).into_response())
}
